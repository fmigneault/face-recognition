...
macro(face_recog_add_sources)
    ...
    set(FaceRecog_HEADER_FILES ${FaceRecog_HEADER_FILES} ${FaceRecog_HEADERS_DIRS}/Utilities/Common.h)
    set(FaceRecog_HEADER_FILES ${FaceRecog_HEADER_FILES} ${FaceRecog_HEADERS_DIRS}/Utilities/ForwardDeclares.h)
    set(FaceRecog_HEADER_FILES ${FaceRecog_HEADER_FILES} ${FaceRecog_HEADERS_DIRS}/Utilities/Macros.h)
...
macro(face_recog_update_modules)
    ...
    if(${FaceRecog_ENABLE_YOLO})
        if(NOT ${WITH_Caffe})
            ...
            set(WITH_Caffe ON CACHE BOOL "Include Caffe library support" FORCE)
            warn("YOLO requires Protobuf, option is enforced")
            set(WITH_Protobuf ON CACHE BOOL "Include Protobuf library support" FORCE)
        endif()
        add_definitions(-DFACE_RECOG_HAS_YOLO)
        add_definitions(-DFACE_RECOG_HAS_CAFFE)
        add_definitions(-DFACE_RECOG_HAS_PROTOBUF)
    else()
        remove_definitions(-DFACE_RECOG_HAS_YOLO)
        remove_definitions(-DFACE_RECOG_HAS_CAFFE)
        remove_definitions(-DFACE_RECOG_HAS_PROTOBUF)
    endif()

    # ESVM
    face_recog_option(BUILD_ESVM "Build Exemplar-SVM (ESVM) module from sources" OFF)
...
macro(face_recog_find_packages)
    ...
    if(${WITH_FlyCapture2})
        ...
        if(${FlyCapture2_FOUND})
            ...
            set(FaceRecog_LIBRARIES_DEBUG   ${FaceRecog_LIBRARIES_DEBUG}    ${FlyCapture2_LIBRARY_DEBUG})
            set(FaceRecog_LIBRARIES_RELEASE ${FaceRecog_LIBRARIES_RELEASE}  ${FlyCapture2_LIBRARY})
        endif()
